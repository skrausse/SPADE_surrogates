...
parser = argparse.ArgumentParser(
    description='Define parameter for filtering the'
                ' results of the SPADE analysis on R2G')

parser.add_argument(
    'context', metavar='context', type=str,
    help='behavioral context (epoch_trialtype) to analyze')
parser.add_argument(
    'session', metavar='session', type=str,
...
if n_surr not in {0, None} and len(ns_sgnt) > 0:
    concepts = list(filter(
        lambda c: spade._pattern_spectrum_filter(concept=c,
                                                 ns_signatures=ns_sgnt,
                                                 spectrum=spectrum,
                                                 winlen=winlen), concepts))
...
if psr_param is not None:
    ...
    pv_spec = None
# Reformatting output
patterns = spade.concept_output_to_patterns(concepts,
                                            pv_spec=pv_spec,
                                            winlen=winlen,
                                            binsize=binsize,
                                            spectrum=spectrum)
